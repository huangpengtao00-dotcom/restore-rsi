...
def task_prompt(task: dict, catalog: str) -> str:
    ...
    return (
        ...
        f"  restore score IN OUT --ref {frame['reference']}\n"
        f"                                 -> compare OUT with the reference; prints REEF_SCORE=<0..1> as its last line\n"
        f"Tools (TOOL):\n{catalog}\n"
        f"Plan the tool sequence from the diagnosis, apply it, then run `restore score` on your final output with IN "
        f"set to the original input path. Finish your reply with that exact REEF_SCORE=... line alone on the last "
        f"line.\n"
        f"Rules (violations score 0): only `restore run` may create or modify images - no python/PIL/numpy/imagemagick "
        f"of your own; never open, copy or inspect the reference image, the task manifest or the degradation code; "
        f"the reference path is only for `restore score --ref`.\n"
