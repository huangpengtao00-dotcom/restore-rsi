...
#### 根因 A:`diagnose` 是 agent 唯一的观测,而它在三个评测任务上全错

启发式诊断的三个读数在**复合退化**下互相污染:

| 任务 | 真实强度 | diagnose 读数 |
|---|---|---|
| haze_low_light_00 | haze 0.699 | **0.073**(低报 10 倍) |
| | low_light 0.813 | **0.194** |
| | blur 不存在 | **0.947**(误报) |
| haze_low_light_01 | haze 0.733 | **0.130** |
| | low_light 0.782 | **0.034**(低报 23 倍) |
| | blur 不存在 | **0.948**(误报) |

机制很清楚,看公式就明白:
- `haze = clip((dark.mean()-0.15)/0.5) * clip(1-lum.std()/0.25)` —— 图一暗,暗通道均值就低,**低光把雾的读数按下去了**;
- `low_light = clip((0.35-lum.mean())/0.35)` —— 雾的大气光会把图**提亮**,于是低光读数被按下去;
- `blur = clip(1-grad.mean()/0.05)` —— 暗图/雾图梯度本来就小,**12 个任务没有一个加过模糊,却有 6 个被报成"强模糊 >0.5"**。

判别力检验:`haze` 读数在场范围 [0.073, 0.755] 与不在场范围 [0.000, 0.202] **重叠**,即这个读数无法判断有没有雾。`low_light` / `noise` 能区分开。

后果在工具使用分布上直接看得到(18 个 episode、207 次调用):

| 工具 | 占比 | 真实情况 |
|---|---|---|
| `sharpen_unsharp` | **23.7%**(第二常用) | 三个评测任务的**任何**最优链里都不出现;单用一次得分 0.0000 |
| `dehaze_dcp` | **9.2%**(最少用) | 两个含雾任务的最优链**第一步**就是它 |

也就是说:**它被告知有模糊(没有)、被告知几乎没有雾(其实很重),于是它去锐化、不去去雾。** 记录段里那条 `sharpen_unsharp>sharpen_unsharp` 拿 0.0000,就是照着错读数执行的结果。
